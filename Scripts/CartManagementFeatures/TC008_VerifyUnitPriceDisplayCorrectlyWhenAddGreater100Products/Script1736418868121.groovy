...
WebUI.setText(findTestObject('DetailPage/txt_Count'), '101')

'Verify the quantity input is updated correctly'
WebUI.verifyMatch(WebUI.getAttribute(findTestObject('DetailPage/txt_Count'), 'value'), '101', false)

'get name product'
productName = WebUI.getText(findTestObject('DetailPage/h3_ProductName')).toLowerCase()
'get unit price when add 99 product'
unitPriceGreater100 = WebUI.getText(findTestObject('DetailPage/div_PriceGreater100'))
...
'verify unit price when add 101 product'
WebUI.verifyMatch(unitPriceGreater100.replaceAll("[^\\d.]", ""), unitPriceCart, false)
